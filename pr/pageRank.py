...
AdjList1 = sc.textFile("a2/al.txt")
print AdjList1.collect()

AdjList2 = AdjList1.map(lambda line : line.split(" "))  # 1.a1=a.map(lambda line : line.split(" "))
...
for i in range(1, 30):
    ...
    contributions = JoinRDD.flatMap(lambda (x, (y, z)) : [(row,z / len(y)) for row in y])  # 4. Replace the lambda function with yours
    print "contributions"
    ...
    PageRankValues = accumulations.mapValues(lambda rank: rank * 0.85 + (0.15/5))  # 6. Replace the lambda function with yours
...
PageRankValues.coalesce(1).saveAsTextFile("a2/PageRankValues_Final")
